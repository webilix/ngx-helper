...
@Directive({ selector: '[ngxUtilsPersianNumber]' })
export class NgxUtilsPersianNumberDirective {
    @Input() ngxUtilsPersianNumber: boolean = true;
    ...
    onKeyPress(event: KeyboardEvent): void {
        if (!this.ngxUtilsPersianNumber) return;
    ...
    @HostListener('input')
    onInput(): void {
        if (!this.ngxUtilsPersianNumber || this.hasMask) return;
        const start: number = this.input.selectionStart || 0;
    ...
    @HostListener('blur')
    onBlur(): void {
        if (!this.ngxUtilsPersianNumber || !this.ngControl || !this.ngControl.control) return;
