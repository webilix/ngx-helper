...
@Directive({ selector: '[ngxHelperParamPersianNumber]' })
export class NgxHelperParamPersianNumberDirective {
    @Input({ required: true }) ngxHelperParamPersianNumber: boolean = true;
    ...
    onKeyPress(event: KeyboardEvent): void {
        if (!this.ngxHelperParamPersianNumber) return;
    ...
    @HostListener('input')
    onInput(): void {
        if (!this.ngxHelperParamPersianNumber || this.hasMask) return;
        const start: number = this.input.selectionStart || 0;
    ...
    @HostListener('blur')
    onBlur(): void {
        if (!this.ngxHelperParamPersianNumber || !this.ngControl || !this.ngControl.control) return;
